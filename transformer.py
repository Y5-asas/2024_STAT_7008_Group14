...
class TranslationDataset_transformer(Dataset):
...
class Train:
    ...
    source_word_2_index: dict = None
    target_word_2_index: dict = None
    N: int = None
    Fixed_len: int = None
    Train_Data: pd.core.frame.DataFrame = None
...
def run_epoch(
    ...
    with open(Train.log_file_path, 'w') as log_f, open(Train.loss_file_path, 'a') as loss_f:
        log_f.write(f"Epoch {TrainState.epoch}/{Train.Epoch}, Training Loss: {avg_loss}\n")
        loss_f.write(f"{TrainState.epoch},{avg_loss}\n")
...
def translate(model, src, source_word_2_index, target_word_2_index, start_symbol):
...
def train():
    ...
    for epoch in range(Train.Epoch):
        ...
        src = Train.Train_Data[Train.src_language][randint]
        print(src)
